...
from mos_sampler import MOSNeighborSamplerTransductive
...
def run(args, device, data):
    multilabel = False # ogbn-products
    ...
    # dataloader
    dataloader = MOSNeighborSamplerTransductive(args.node_budget, args.num_layers, train_g, train_mask, args.decomp)
    ...
    for epoch in range(args.num_epochs):
        ...
        if epoch % args.eval_every == 0:
            eval_acc, test_acc = evaluate(model, test_g, test_nfeat, test_labels, 
                    [val_nid, test_nid], device, multilabel)
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--node-budget', type=int, default=30000)
    ...
    argparser.add_argument('--valbs', type=int, default=10000) # validation batch size
    ...
    g = dgl.add_self_loop(g) # add self loop to support gcn
